...
it('renders the challenge copy and shared form resource', () => {
  render(<TwoFactorChallengePage challengeForm={form} />)
...
it('renders setup state without inventing a second form schema', () => {
  render(<TwoFactorSettingsPage status={{ enabled: false }} enrollmentForm={form} confirmForm={form} />)
...
it('renders the enrollment URI and recovery codes', () => {
  render(<TwoFactorSettingsPage
    ...
    enrollmentQrCode="data:image/svg+xml,%3Csvg%3E%3C/svg%3E"
    enrollment={{ secret: 'SECRET', otpauthUri: 'otpauth://totp/Inlay:test@example.com', recoveryCodes: ['FIRST-CODE'] }}
  />)
...
it('renders recovery rotation and disable actions after setup', () => {
  render(<TwoFactorSettingsPage status={{ enabled: true }} enrollmentForm={form} confirmForm={form} regenerateForm={form} disableForm={form} />)
